...
export class RequireCadastro {
  setColaborador(obj) {
    if (obj.state.senha !== obj.state.comSenha) {
      obj.setState({
        mensagemModalError: "Senhas Diferentes",
      ...
      obj.setState({
        mensagemModalError: "Alguns Campos não foram Preenchidos",
        modalStateError: true,
      });
      return;
    }
    ...
    obj.setState({
      mensagemModal: "Colaborador cadastrado com sucesso!",
      modalState: true,
    });
    try {
  ...
  async enviarFotoColaborador(obj) {
    ...
    const requestFile = {
      ...
      ext: `${primeroNome(obj.state.nomeCompleto)} ${ultimoNome(
        obj.state.nomeCompleto
      )}${exten}`,
    };
    await axios.post(
