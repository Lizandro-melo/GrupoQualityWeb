...
import { dataHoje, formatarData } from "../../Anotacoes/Constant/AnotacoesConstant"
import { diaNascimneto, mesNascimento, primeiroUltimoNome } from "../../Cadastrar/Constants"
import Cookies from "js-cookie"

export class RequireAtualizar {

    getDadosColaborador(obj, idColaborador) {
        axios.post(`https://localhost:8081/rh/colaboradores/id?id=${idColaborador}`).then((element) => {
            ...
            axios.post(`https://localhost:8081/intranet/colaborador/nome?nome=${colaborador.nome.toLowerCase()}`).then((element) => {
                const colaborador = element.data[0]
                if (colaborador === undefined) {
                    return
                }
                obj.setState({
                    permissaoPrimaria: colaborador.rolePrimary === undefined ? null : colaborador.rolePrimary,
                    permissaoSecundaria: colaborador.roleSecondary === undefined ? null : colaborador.roleSecondary,
    ...
    verificarCampos(obj) {
        ...
        if (obj.state.senha === "" || obj.state.comSenha === "" || obj.state.dataNascimento === "" || obj.state.empresa === "" || obj.state.setor === "" || obj.state.tipo === "" || obj.state.permissaoPrimaria === "" || obj.state.permissaoSecundaria === "") {
            obj.setState({
    ...
    contratarEstagiario(obj, idColaborador) {
        const reactObj = obj.state
        ...
        axios.put(`https://localhost:8081/rh/contratar/estagiario?id=${idColaborador}&data=${dataHoje}`).then(response => response.data.nome);
        obj.setState({
    ...
    desligarColaborador(obj, idColaborador){
        ...
        axios.put(`https://localhost:8081/rh/colaborador/desligar?id=${idColaborador}&data=${dataHoje}&responsavel=${user.nome} ${user.sobrenome}`).then(response => response.data.nome);
        obj.setState({
    ...
    async updateColaborador(obj) {
        ...
        const colaboradorRh = {
            ...
            dataAdmissao: obj.state.dataInicio,
            dataDemissao: obj.state.dataDemissao
        }
        if (obj.state.tipo === "CONTRATADO") {
            axios.put(`https://localhost:8081/rh/colaboradores/update?setor=${obj.state.setor}&empresa=${obj.state.empresa}`, colaboradorRh)
        } else if (obj.state.tipo === "ESTAGIARIO") {

        }
        axios.put(`https://localhost:8081/rh/colaboradores/intranet/update`, colaboradorIntra)
        obj.setState({
