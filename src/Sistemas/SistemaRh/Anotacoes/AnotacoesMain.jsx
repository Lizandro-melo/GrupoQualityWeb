...
export default class AnotacoesMain extends Component {
  ...
  state = {
    ...
    colaboradoresList: [],
    anotacoesList:
      JSON.parse(localStorage.getItem("anotacoesList")) === null
        ? []
        : JSON.parse(localStorage.getItem("anotacoesList")),
    //dados Calculados
  ...
  abrirModalCriarAnotacao = () => {
    if (this.state.colaborador !== "") {
      ...
      this.abrirModalMensagem(
        "Selecione um Colaborador antes de tentar criar uma anotação"
      );
    }
  };
  ...
  render() {
    return (
      <section className=" flex gap-5 w-full">
        ...
        >
          <section className="w-5/6 max-lg:w-11/12 flex flex-col h-5/6 kanit bg-stone-50 rounded-xl shadow-xl">
            <header className="w-full bg-blue-950 h-10 rounded-t-xl flex items-center justify-between px-10">
        ...
        <ReactModal
          ...
          style={{ overlay: { zIndex: 1000 }, content: { zIndex: 1001 } }}
        >
          <section className="w-3/5 flex flex-col h-2/5 kanit bg-stone-50 rounded-xl shadow-xl">
            <header className="w-full bg-blue-950 h-10 rounded-t-xl flex items-center justify-between px-10">
              <span className="text-white kanit font-semibold">
                Mensagem de Erro
              </span>
        ...
        <section className="flex flex-col w-full">
          <section
            className={`bg-white flex justify-start gap-3 items-center p-5 rounded-2xl border-b-4 ${
          ...
          >
            ...
            <section className="flex flex-col text-xs">
              ...
              <span className="font-semibold">
                Data de Nascimento: {this.state.colaborador.dataNascimento}
              </span>
            </section>
          </section>
          <section className="flex relative">
            ...
            >
              <section className="flex relative items-center">
                  ...
                  <section className="flex gap-4 max-lg:flex-wrap max-lg:w-52 max-lg:gap-2 max-lg:justify-end">
                      ...
                      <input
                        onChange={() => {
                          this.setState({
                            ...
                            selectTipoColaborador: "CONTRATADO",
                          });
                          this.setState({});
                      ...
                      <input
                        onChange={() => {
                          this.setState({
                            ...
                            selectTipoColaborador: "ESTAGIARIO",
                          });
                          localStorage.setItem(
                            "selectTipoColaborador",
                            "ESTAGIARIO"
                        ...
                        type="radio"
                        checked={
                          this.state.filtroColaborador === "ESTAGIARIOS"
                            ? true
                            : false
                        }
                      />
                    ...
                    <section className="flex gap-1 items-center">
                      ...
                      <input
                        ...
                        type="radio"
                        checked={
                          this.state.filtroColaborador === "DESLIGADOS"
                            ? true
                            : false
                        }
                      />
                ...
                <button
                  type="button"
                  className="absolute top-10 right-14 bg-white transition-colors border border-stone-300 px-8 h-8 rounded-md cursor-pointer hover:bg-stone-200 flex justify-center items-center font-medium"
              ...
              <section className="flex flex-col gap-4">
                ...
                <section className="flex flex-col gap-2">
                  ...
                  <select
                    ...
                    value={this.state.selectTipoColaborador}
                  >
                    <option value={this.state.colaborador.tipo}>
            ...
            <section className="flex bottom-5 -right-5 absolute justify-end items-end">
              <section className="flex border w-56 rounded-s-xl justify-center border-stone-300 border-r-white bg-white ">
                <table className="w-52">
                  ...
                  <tr>
                    <td>Advertências Escritas:</td>
                    <td>{this.state.advsEscritas}</td>
                  </tr>
                  <tr>
                    <td>Advertências Verbais:</td>
          ...
          <section className="p-4 w-full flex gap-3 flex-col justify-between bg-white border rounded-xl border-stone-300 overflow-auto max-h-96">
            ...
            <section className="flex-col-reverse w-full  flex rounded-xl gap-3">
              {this.state.anotacoesList.map((anotacao) => {
                return (
                  <section
                    className={`px-3 py-3 flex flex-col relative  rounded-xl border-b-4 border w-full cursor-pointer h-24 justify-between border-blue-400 ${
                      anotacao.status === false ? "opacity-30" : ""
                    }`}
                    onClick={() => {
                      localStorage.setItem(
                        "anotacao",
                    ...
                    title={
                      anotacao.status === false
                        ? "Anotações Excluidas não são contabilizadas!"
                        : ""
                    }
                  >
                    ...
                    <section className="overflow-hidden max-w-sm text-ellipsis whitespace-nowrap font-bold ">
                      <span className="text-sm ">{anotacao.anotacao}</span>
                    </section>
                  </section>
                );
              })}
