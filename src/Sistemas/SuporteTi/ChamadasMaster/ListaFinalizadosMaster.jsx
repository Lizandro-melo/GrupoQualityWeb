...
import {
  RequestFechadosComum,
  RequestFechadosMaster,
} from "../../../Controller/ticket/TickeatFechados";

export default function ListaFinalizadosMaster() {
  const [finalizado, setFinalizados] = useState([]);
  ...
  useEffect(() => {
    if (user.roleSecondary === "MASTER") {
      setInterval(() => {
        RequestFechadosMaster(setFinalizados);
      }, 500);
    } else {
      setInterval(() => {
        RequestFechadosComum(setFinalizados);
      }, 500);
    }
  }, [setFinalizados, user.roleSecondary]);
  ...
  return (
    <>
      ...
      <section className="bg p-7">
        <span className="text-xl font-semibold">FINALIZADOS</span>
      </section>
      <hr className="" />
      <section className="w-full max-h-96 px-2 py-3 rounded-b-xl overflow-auto">
        <table className="justify-center items-center max-lg:items-start flex flex-col">
          <tbody className="text-sm flex-none">
            <tr>
