...
import {
  RequestPendenteMaster,
  RequestPendenteComum,
} from "../../../Controller/ticket/TicketPendente";
import {
  DeletarTicket,
  EnviarParaAndamento,
...
export default function ListaPendentesMaster() {
  ...
  useEffect(() => {
    if (user.roleSecondary === "MASTER") {
      setInterval(() => {
        RequestPendenteMaster(setPendentes);
      }, 500);
    } else {
      setInterval(() => {
        RequestPendenteComum(setPendentes);
      }, 500);
    }
  }, [setPendentes, user.roleSecondary]);
